...
scib_summary_plot <- scib_summary |>
  ...
  dplyr::mutate(id = as.character(seq_len(nrow(scib_summary)))) |>
  ...
  # Create rank labels
  dplyr::mutate(
    label_pancreas = dplyr::case_match(
      rank(rank_pancreas), # We re-rank just the top performing methods
      1 ~ "1",
      2 ~ "2",
      3 ~ "3"
    ),
    label_lung_atlas = dplyr::case_match(
      rank(rank_lung_atlas),
      1 ~ "1",
      2 ~ "2",
      3 ~ "3"
    ),
    label_immune_cell_hum = dplyr::case_match(
      rank(rank_immune_cell_hum),
      1 ~ "1",
      2 ~ "2",
      3 ~ "3"
    ),
    label_immune_cell_hum_mou = dplyr::case_match(
      rank(rank_immune_cell_hum_mou),
      1 ~ "1",
      2 ~ "2",
      3 ~ "3"
    ),
    label_mouse_brain = dplyr::case_match(
      rank(rank_mouse_brain),
      1 ~ "1",
      2 ~ "2",
      3 ~ "3"
    ),
    label_simulations_1_1 = dplyr::case_match(
      rank(rank_simulations_1_1),
      1 ~ "1",
      2 ~ "2",
      3 ~ "3"
    ),
    label_simulations_2 = dplyr::case_match(
      rank(rank_simulations_2),
      1 ~ "1",
      2 ~ "2",
      3 ~ "3"
    ),
    package_label = dplyr::case_match(
      rank(-package_rank, ties.method = "min"),
      1 ~ "1",
      2 ~ "2",
      3 ~ "3"
    ),
    paper_label = dplyr::case_match(
      rank(-paper_rank, ties.method = "min"),
      1 ~ "1",
      2 ~ "2",
      3 ~ "3"
    ),
    time_label = dplyr::case_match(
      rank(-time_rank, ties.method = "min"),
      1 ~ "1",
      2 ~ "2",
      3 ~ "3"
    ),
    memory_label = dplyr::case_match(
      rank(-memory_rank, ties.method = "min"),
      1 ~ "1",
      2 ~ "2",
      3 ~ "3"
    )
  ) |>
  # Subset to the columns to plot
  dplyr::select(
    id,
    method,
    output_img,
    ...
    label_immune_cell_hum,
    overall_immune_cell_hum_mou,
    label_immune_cell_hum_mou,
    overall_mouse_brain,
    label_mouse_brain,
...
dplyr::glimpse(scib_summary_plot)
```
...
column_info <- tibble::tribble(
      ...
      "overall_immune_cell_hum",       "Immune\n(human)",   "bar",         "RNA",   "blues",     1.5,     TRUE,      NA,     FALSE,
        "label_immune_cell_hum",                      NA,  "text",         "RNA",        NA,      NA,    FALSE,     0.1,      TRUE,
  "overall_immune_cell_hum_mou", "Immune\n(human/mouse)",   "bar",         "RNA",   "blues",     1.5,     TRUE,      NA,     FALSE,
